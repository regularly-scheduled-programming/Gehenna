...
{
	return PostAkEventAndWaitForEnd(AkAudioEvent, FString(), LatentInfo);
}
...
{
	return PostAkEventByNameWithDelegate(GET_AK_EVENT_NAME(AkEvent, in_EventName), CallbackMask, PostEventCallback);
}
...
AkPlayingID UAkComponent::PostAkEventByNameWithDelegate(const FString& in_EventName, int32 CallbackMask, const FOnAkPostEventCallback& PostEventCallback)
{
...
int32 UAkComponent::PostAkEventAndWaitForEnd(class UAkAudioEvent * AkEvent, const FString& in_EventName, FLatentActionInfo LatentInfo)
{
	...
	if (AudioDevice && CurrentWorld)
	{
		...
		AkPlayingID PlayingID = AudioDevice->PostEventLatentAction(AkEvent->GetName(), this, NewAction);
